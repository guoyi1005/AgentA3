...
test('AIPPT uses real outline, slide, task, progress, preview and download APIs', () => {
  ...
  for (const name of [
    'createPptOutlineTask', 'createPptSlidesTask', 'createPptTask', 'streamPptTask',
  ...
  assert.match(page, /exportFormats:\s*\['pptx'\]/)
  ...
  assert.match(container, /'presentation', 'ppt', 'aippt'/)
  assert.match(container, /templateFirst = requestedEntry === 'templateFirst'/)
...
test('AIPPT exposes the backend template catalog and sends the selected template through generation', () => {
  const page = source('resourceGenerate/AIPresentationFlow.vue')
  assert.match(page, /template-library-entry/)
  assert.match(page, /template-usage-card__actions/)
  assert.match(page, /更换模板/)
  assert.doesNotMatch(page, /templateExpanded/)
  assert.doesNotMatch(page, /<view class="template-section-head">/)
  assert.doesNotMatch(page, /<scroll-view[\s\S]*class="style-scroll"/)
  ...
  assert.match(page, /使用并生成大纲/)
  assert.match(page, /templateReturnContext: null/)
  assert.match(page, /rememberTemplateReturnContext\(\)/)
  assert.match(page, /restoreTemplateReturnContext\(\)/)
  assert.match(page, /returnFromTemplateSelection\(\)/)
  assert.match(page, /templateReturnContext\?\.entryMode !== 'upload'/)
  assert.match(page, /shortTitle:\s*'上传资料'[\s\S]*shortTitle:\s*'选模板'[\s\S]*shortTitle:\s*'编辑大纲'/)
  assert.match(page, /shortTitle:\s*'选模板'[\s\S]*shortTitle:\s*'上传资料'[\s\S]*shortTitle:\s*'编辑大纲'/)
  assert.doesNotMatch(page, /shortTitle:\s*'大纲来源'/)
...
test('AIPPT upload and manual input come before template selection and keep actions floating', () => {
  ...
  assert.match(page, /\[1,\s*2,\s*3,\s*4,\s*5,\s*6,\s*7,\s*8\]\.includes\(this\.currentStep\)/)
  ...
  assert.match(page, /outline-mode-selector/)
  assert.match(page, /outline-mode-card/)
  assert.match(page, /outline-mode-card--selected/)
  assert.match(page, /outline-mode-card__radio-dot/)
  assert.match(page, /选择上传类型/)
  assert.match(page, /非大纲资料/)
  assert.match(page, /上传课件、笔记或成套资料，系统将自动整理大纲/)
  assert.match(page, /上传大纲/)
  assert.match(page, /已准备好提纲或目录时可直接上传，尽量保留原结构/)
  assert.doesNotMatch(page, /outline-mode-inline/)
  assert.doesNotMatch(page, /outline-mode-chip/)
  assert.match(page, /return this\.prepareOutline\(\)/)
  assert.doesNotMatch(page, /先决定大纲来源/)
  assert.doesNotMatch(page, /mode-intro/)
  assert.match(page, /outline-toolbar-actions/)
  ...
  assert.match(page, /v-if="index < outlineItems\.length - 1" @tap="moveOutlineItem\(index, 1\)">下移/)
  ...
  assert.match(page, /slide-layout-lock-card/)
  assert.match(page, /锁定当前页面/)
  assert.match(page, /toggleActiveSlideLock/)
  assert.match(page, /layoutLocked: Boolean\(source\.layoutLocked\)/)
  assert.match(page, /layoutLocked: Boolean\(slide\.layoutLocked\)/)
  assert.match(page, /生成时保留当前预览版式/)
...
test('AIPPT asks before restoring unfinished input and exposes a one-tap restart', () => {
  const page = source('resourceGenerate/AIPresentationFlow.vue')
  assert.match(page, /重新输入/)
  assert.match(page, /requestRestartFlow/)
  assert.match(page, /promptSavedWork/)
  assert.match(page, /发现未完成内容/)
  assert.match(page, /confirmText: '继续'/)
  assert.match(page, /cancelText: '重新输入'/)
  assert.match(page, /aiPptDraft/)
  assert.match(page, /persistDraft/)
  assert.match(page, /restoreDraft/)
  assert.match(page, /resetFlowState/)
  assert.match(page, /uni\.removeStorageSync\('aiPptDraft'\)/)
  assert.match(page, /uni\.removeStorageSync\('aiPptActiveTask'\)/)
  assert.match(page, /this\.persistDraft\(\)[\s\S]*this\.clearTimers\(\)/)
  assert.doesNotMatch(page, /this\.restoreActiveTask\(\)\s*\n/)
})

test('AIPPT keeps the last successful result and confirms before regenerating edits', () => {
  const page = source('resourceGenerate/AIPresentationFlow.vue')
  assert.match(page, /lastSuccessfulResult: null/)
  assert.match(page, /editorDirty: false/)
  assert.match(page, /captureSuccessfulResult/)
  assert.match(page, /restoreLastSuccessfulResult/)
  assert.match(page, /returnToLastSuccessfulResult/)
  assert.match(page, /当前页面内容已修改，确认重新生成 PPT 吗？/)
  assert.match(page, /confirmText: '确认生成'/)
  assert.match(page, /cancelText: '继续编辑'/)
  assert.match(page, /当前修改还没有重新生成，返回后将放弃这些修改/)
  assert.match(page, /是否返回上次成功生成的 PPT？/)
  assert.match(page, /hadPreviousResult = this\.hasLastSuccessfulResult/)
  assert.match(page, /继续编辑/)
  assert.match(page, /goToSettingsForRegeneration/)
  assert.match(page, /this\.saveSuccessfulResult\(\)/)
  assert.match(page, /lastSuccessfulResult: this\.lastSuccessfulResult/)
  assert.match(page, /editorPrimaryLabel\(\)[\s\S]*确认修改并生成/)
  assert.match(page, /runGeneration\(\)/)
  assert.doesNotMatch(page, /JSON\.stringify\(this\.buildTaskPayload\(\)\) === this\.completedTaskFingerprint/)
  assert.match(page, /slidesDirty: false/)
  assert.match(page, /canReturnToEditor\(\)/)
  assert.match(page, /settingsNextLabel\(\)/)
  assert.match(page, /handleSettingsNext\(\)/)
  assert.match(page, /重新生成页面内容/)
  assert.match(page, /@tap="handleSettingsNext"/)
  assert.match(page, /outlineSourceDirty: false/)
  assert.match(page, /canReturnToOutline\(\)/)
  assert.match(page, /handleOutlineEntryNext\(\)/)
  assert.match(page, /if \(this\.canReturnToOutline\) \{[\s\S]*this\.currentStep = 3/)
  assert.match(page, /资料已修改/)
  assert.match(page, /检测到上传资料或大纲来源已修改，是否重新生成大纲？/)
  assert.match(page, /confirmText: '重新生成'/)
  assert.match(page, /cancelText: '返回原大纲'/)
  assert.match(page, /this\.outlineSourceDirty = false[\s\S]*this\.currentStep = 3/)
  assert.match(page, /返回大纲/)
  assert.match(page, /重新生成大纲/)
  assert.match(page, /@tap="selectOutlineMode\(mode\.id\)"/)
  assert.match(page, /markOutlineSourceDirty\(\)/)
  assert.match(page, /outlineSourceDirty: this\.outlineSourceDirty/)
  assert.match(page, /@tap="navigateToStep\(item\.id\)"/)
  assert.match(page, /isStepAvailable\(item\.id\)/)
  assert.match(page, /return this\.isStepAvailable\(target\)/)
  assert.match(page, /可进入/)
  assert.match(page, /isStepNavigable\(item\)/)
  assert.match(page, /navigateToStep\(step\)/)
  assert.match(page, /当前任务进行中，请先取消任务/)
  assert.match(page, /stepper__item--clickable/)
})
